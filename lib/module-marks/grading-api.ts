 ...
 */
export async function approveOverallMarksByHOD(moduleId: string, data: OverallApprovalData): Promise<any> {
  try {
    const response = await api.post(`/grading/marks-submission/module/${moduleId}/hod/approve-overall`, data)
...
export const fetchModuleMarkSheetExcel = async (moduleId: string): Promise<Blob> => {
  try {
    const response = await api.get(`/grading/student-marks/module/${moduleId}/generateModuleMarkSheetExcel`, {
      responseType: "blob",
      timeout: 30000,
    })
    return response.data
...
export const fetchModuleExamSheetExcel = async (moduleId: string): Promise<Blob> => {
  try {
    const response = await api.get(`/grading/exam-marks/module/${moduleId}/generate-exam-sheet`, {
      responseType: "blob",
      timeout: 30000,
    })
    return response.data
...
export const approveCATMarksByHOD = async (moduleId: string): Promise<any> => {
  try {
    ...
    const response = await api.post(`/grading/marks-submission/module/${moduleId}/hod/approve-cat`, requestBody, {
      timeout: 30000,
      headers: {
    ...
    if (error.response?.status === 404) {
      ...
      const errorMsg = error.response?.data?.message || error.message || "Failed to approve CAT marks"
...
export const approveExamMarksByHOD = async (moduleId: string): Promise<any> => {
  try {
    ...
    const response = await api.post(`/grading/marks-submission/module/${moduleId}/hod/approve-exam`, requestBody, {
      timeout: 30000,
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (error.response?.status === 404) {
      ...
      const errorMsg = error.response?.data?.message || error.message || "Failed to approve exam marks"
      throw new Error(`${errorMsg} (Status: ${error.response?.status || "Unknown"})`)
    }
  }
}
...
export const submitToDean = async (groupId: string, semesterId: string, submissionNotes?: string): Promise<any> => {
  try {
