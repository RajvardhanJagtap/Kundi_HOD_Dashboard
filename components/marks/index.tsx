...
// Define MarksRow type (adjust fields as needed)
...
// Placeholder for MarksSubmittedTable component (replace with actual implementation if needed)
export const MarksSubmittedTable: React.FC<{
  data: MarksRow[]
  page: number
  pageSize: number
  onPageChange: (p: number) => void
  totalPages: number
  renderActions: (row: MarksRow) => React.ReactNode
}> = ({ data, page, pageSize, onPageChange, totalPages, renderActions }) => {
  return (
    <div>
      {/* Table rendering logic goes here */}
      <p>MarksSubmittedTable placeholder</p>
    </div>
  )
}
...
export const MarksModuleSection: React.FC<{
  ...
  const statusOptions = ["All Status", "Pending", "Approved", "Overdue", "Rejected"]
      ...
      <div className="flex gap-2 mb-4 items-center">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="flex items-center gap-2 min-w-[120px] justify-between bg-transparent">
          ...
          <DropdownMenuContent align="end" className="">
            {statusOptions.map((status) => (
              <DropdownMenuItem
                ...
                className={`flex items-center gap-2 px-2 py-2 ${selectedStatus === status ? "bg-accent" : ""}`}
              >
                <span>{status}</span>
                {selectedStatus === status && <span className="ml-auto text-xs text-muted-foreground">Selected</span>}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
  ...
  return (
    <>
              ...
              <TableRow key={row.id || idx}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                        row.statusColorCode === "orange"
                          ? "bg-orange-100 text-orange-800"
                          : row.statusColorCode === "green"
                            ? "bg-green-100 text-green-800"
                            : row.statusColorCode === "red"
                              ? "bg-red-100 text-red-800"
                              : row.statusColorCode === "blue"
                ...
                <TableCell className="text-right flex gap-2 justify-end">
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        `/registrar-academics/marks-submitted/class/${encodeURIComponent(row.className)}/2025`,
                      )
                    }
                  >
                    <svg
      ...
      <div className="flex justify-end items-center mt-4 gap-2">
        ...
        <button
          className="px-3 py-1 rounded-md border text-sm font-medium bg-white text-gray-700 disabled:opacity-50"
          onClick={() => setClassPage(Math.min(classTotalPages, classPage + 1))}
    ...
    <div>
      ...
      <div className="space-y-3">
        {deadlinesData.map((deadline, index) => (
          <div
            key={index}
            className={`p-4 rounded-lg border ${deadline.status === "Overdue" ? "bg-red-50 border-red-200" : "bg-yellow-50 border-yellow-200"}`}
          >
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                <span
                  className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${deadline.status === "Overdue" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}
                >
                  {deadline.status}
                </span>
              </div>
