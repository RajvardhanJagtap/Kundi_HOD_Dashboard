...
export default function SummaryPage({
  ...
  const loadPreview = async () => {
    ...
    try {
      if (!academicYearId || !groupId) {
        throw new Error(
          "Academic Year ID and Group ID are required for summary sheet generation."
        );
      }

      const previewData = await fetchAndParseSummarySheet(
        academicYearId,
        groupId
      );
  ...
  useEffect(() => {
    ...
    if (academicYearId && groupId) {
      console.log("Summary component - Loading preview with:", {
        academicYearId,
        groupId,
      });
      void loadPreview();
    } else {
      console.warn("Summary component - Missing required IDs:", {
        academicYearId,
        groupId,
      });
    }
  ...
  useEffect(() => {
    if (academicYearId && groupId && !isPreviewLoading && !previewData) {
      console.log("Summary component - Props changed, loading preview:", {
        academicYearId,
        groupId,
      });
      void loadPreview();
          ...
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-gray-500 mb-4">
                The summary sheet preview will load automatically when data is
                available.
              </p>
              <Button
                onClick={loadPreview}
