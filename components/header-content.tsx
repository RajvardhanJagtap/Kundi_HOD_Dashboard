...
export function HeaderContent({ children }: { children: React.ReactNode }) {
  ...
  // Keep the mobile sidebar positioned below the full header (toolbar + dropdown row).
  // Set as a global CSS variable so it can be read by the sidebar overlay.
  useEffect(() => {
  ...
  useEffect(() => {
    if (
      ...
      const mostRecentYear = years.reduce((prev, current) => {
        return !prev || new Date(current.endDate) > new Date(prev.endDate)
          ? current
          : prev;
      });
  ...
  useEffect(() => {
    if (
      ...
      const hasValidSemester =
        selectedSemester && semesters.some((s) => s.id === selectedSemester);
  ...
  return (
    <SidebarInset className="flex flex-col h-screen overflow-hidden min-w-0 overscroll-none">
      <header ref={headerRef} className="flex-shrink-0 z-20 border-b bg-white">
        <div className="flex h-16 items-center justify-between px-4 md:px-6">
          {/* Left: Sidebar + Brand */}
          <div className="flex items-center gap-3 min-w-0">
            <SidebarTrigger className="h-10 w-10 rounded-lg bg-gray-100 text-gray-800 hover:bg-gray-200 md:hidden" />
            <span className="font-bold text-xl text-[#026892] tracking-tight truncate">
              SAMPS UR
            </span>
          </div>

          {/* Right: (Desktop) selectors + role, notifications, user */}
        ...
        {/* Mobile: selectors row under header (matches screenshot layout) */}
      ...
      <main className="flex-1 min-h-0 overflow-auto p-4 md:p-6 bg-gray-50 min-w-0 overscroll-none touch-pan-y">
        <div className="min-h-full flex flex-col">
          <div className="flex-1 min-w-0">{children}</div>
          <footer className="mt-8 flex flex-col gap-2 sm:flex-row py-6 w-full items-center border-t text-xs text-gray-500 bg-white">
            <p>
              &copy; {new Date().getFullYear()} SAMPS UR. All rights reserved.
            </p>
