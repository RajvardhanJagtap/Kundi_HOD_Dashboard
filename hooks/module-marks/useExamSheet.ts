...
function parseMergeRanges(worksheet: ExcelJS.Worksheet): MergeRange[] {
  ...
  console.log("[v0] Exam sheet parsed merges:", merges)
  return merges
}
...
export function useExamSheet(moduleId: string | null) {
  ...
  const fetchExamSheet = useCallback(async () => {
    ...
    try {
      ...
      setExamSheetData(rawData)
      setStyledData(styled)
    } catch (err) {
      console.error("Error fetching exam sheet:", err)
